import os
import glob
import psycopg2
import pandas as pd
import sql_queries


def process_song_file(cur, filepath):
    '''
        This function separates out the song and artist dataframes and insert them into
        relevant postgres tables.

        Parameters
        ----------
        cur : str
            Cursor of the database connection.
        filepath : str
            Path of the file to be processed.

        Raises
        ------
        Exception
            If an error occurred while processing the file.
    '''
    try:
        # open song file
        df = pd.read_json(filepath, lines=True)

        # insert song record
        song_data = list(
            df[['song_id', 'title', 'artist_id', 'year', 'duration']].values[0])

        cur.execute(sql_queries.song_table_insert, song_data)

        # insert artist record
        artist_data = list(df[['artist_id', 'artist_name', 'artist_location',
                               'artist_latitude', 'artist_longitude']].values[0])
        cur.execute(sql_queries.artist_table_insert, artist_data)

    except Exception as e:
        print(f"An error occurred while processing the file: {e}")


def process_log_file(cur, filepath):
    '''
        This function separates out the time, user and song-play dataframes and
        insert them into the relevant postgres tables.

        Parameters
        ----------
        cur : str
            Cursor of the database connection.
        filepath : str
            Path of the file to be processed.

        Raises
        ------
        Exception
            If an error occurred while processing the file.
    '''
    try:
        # open log file
        df = pd.read_json(filepath, lines=True)

        # filter by NextSong action
        df = df.loc[df['page'] == 'NextSong']

        # convert timestamp column to datetime
        t = pd.to_datetime(df['ts'], unit='ms')

        # insert time data records
        time_data = [df.ts.values, t.dt.hour.values, t.dt.day.values,
                     t.dt.isocalendar().week.values, t.dt.month.values, t.dt.year.values,
                     t.dt.weekday.values]
        column_labels = {'start_time', 'hour', 'day', 'week', 'month', 'year', 'weekday'}
        time_df = pd.DataFrame(dict(zip(column_labels, time_data)))

        for i, row in time_df.iterrows():
            cur.execute(sql_queries.time_table_insert, list(row))

        # load user table
        user_df = df[['userId', 'firstName', 'lastName', 'gender', 'level']]

        # insert user records
        for i, row in user_df.iterrows():
            cur.execute(sql_queries.user_table_insert, row)

        # insert songplay records
        for index, row in df.iterrows():

            # getsongid and artistid fromsong and artist tables
            cur.execute(sql_queries.song_select, (row.song, row.artist, row.length))
            results = cur.fetchone()

            if results:
                songid, artistid = results
            else:
                songid, artistid = None, None

            # insertsongplay record
            songplay_data = [row.ts, row.userId,
                             row.level, songid, artistid, row.sessionId,
                             row.location, row.userAgent]
            cur.execute(sql_queries.songplay_table_insert, songplay_data)
    except Exception as e:
        print(f"An error occurred while processing the file: {e}")


def process_data(cur, conn, filepath, func):
    '''
        This function iterates over all the files and
        then call the process_song_file and process_log_file functions

        Parameters
        ----------
        cur : str
            Cursor of the database connection.
        conn: str
            Connection of the database.
        filepath : str
            Path of the file to be processed.
        func : str
            name of the function to be called
    '''
    # get all files matching extension from directory
    all_files = []
    for root, dirs, files in os.walk(filepath):
        files = glob.glob(os.path.join(root, '*.json'))
        for f in files:
            all_files.append(os.path.abspath(f))

    # get total number of files found
    num_files = len(all_files)
    print('{} files found in {}'.format(num_files, filepath))

    # iterate over files and process
    for i, datafile in enumerate(all_files, 1):
        func(cur, datafile)
        conn.commit()
        print('{}/{} files processed.'.format(i, num_files))


def main():
    '''
        This function connects to the postgres Database called "sparkifydb". It creates a
        connection and cursor. After that it calls the process_data function for song data
        and log data. Finally, it closes the connection.
    '''
    conn = psycopg2.connect("host=127.0.0.1 dbname=sparkifydb user=student password=student")
    cur = conn.cursor()

    process_data(cur, conn, filepath='data/song_data', func=process_song_file)
    process_data(cur, conn, filepath='data/log_data', func=process_log_file)

    conn.close()


if __name__ == "__main__":
    main()
